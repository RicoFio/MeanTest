...
def clean_list(norm, pass_check, comets):
    ...
    if len(pass_check) > 1:
        ...
        acceppted_normalized_matrix =[set1_values,set2_values]
    #print pass_check
...
def save_results(wb1,avg_set_1, avg_set_2, n, pc):
    ...
    for i,set_d in enumerate(n):
        wb1['A{}'.format(idx)] = "Set {}".format(i+1)
        idx += 1
...
if __name__ == '__main__':
    ...
    elif len(pass_check) == 1:

        averages_set_1 = final_norm[0][0]
        averages_set_2 = final_norm[1][0]
        #print averages_set_1
        #print averages_set_2
    
    elif len(pass_check) == 0:
        
        print "sorry but this dataset cannot be regarded"
        sys.exit()
    # Create new work sheet
    ws1 = wb.create_sheet("Output_Data")
    # give result
    print_results(averages_set_1, averages_set_2)
    save_results(ws1, averages_set_1, averages_set_2, norm, pass_check)
